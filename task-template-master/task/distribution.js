...
const tokenContractABI = require('../abi/LensLink.json');
const { retrieveFromCid } = require('../helpers');
const web3 = new Web3(`https://polygon-amoy.infura.io/v3/${process.env.INFURA_ID}`);
const tokenContractAddress = process.env.TOKEN_CONTRACT_ADDRESS;
const privateKey = process.env.PRIVATE_KEY;

async function distributeLensLink(distributionCandidates, submissions) {
...
class Distribution {
  ...
  async generateDistributionList(round, _dummyTaskState) {
    try {
      ...
      await distributeLensLink(distributionCandidates, submissions)
      return distributionList;
    } catch (err) {
